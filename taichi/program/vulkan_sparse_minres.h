...
// symmetric operators. Lanczos/Givens scalars stay in one packed device
// state, the submission sequence is bounded and masked, and each completed
// iteration is verified with a true residual before the single final readback.
class VulkanSparseMINRESPlan {
 public:
  VulkanSparseMINRESPlan(Program *program,
                         SparseMatrix &matrix,
                         int max_iterations,
                         float absolute_tolerance,
                         bool verbose,
                         float relative_tolerance = 0.0f);
  ~VulkanSparseMINRESPlan();
 ...
 private:
  ...
  Ndarray *v_old_{nullptr};
  Ndarray *v_{nullptr};
  Ndarray *v_new_{nullptr};
  Ndarray *p_older_{nullptr};
  Ndarray *p_old_{nullptr};
  Ndarray *p_{nullptr};
