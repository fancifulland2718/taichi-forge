...
constexpr int kStateWords = 24;
constexpr int kStateBytes = kStateWords * sizeof(std::uint32_t);
constexpr int kTrueResidualSquared = 10;
constexpr int kInitialResidualSquared = 11;
constexpr int kRelativeReferenceNorm = 13;
constexpr int kEffectiveTolerance = 14;
constexpr int kStatus = 16;
constexpr int kCompletedIterations = 17;
constexpr int kActive = 18;
...
#if defined(TI_WITH_VULKAN)
void push_bicgstab_resource(VulkanCommandReplayKey &key,
                            const Ndarray *array) {
  if (!array) {
    for (int i = 0; i < 4; ++i) {
      key.push(0);
...
void push_bicgstab_stamp(VulkanCommandReplayKey &key,
  ...
  struct Key {
    ...
    bool operator==(const Key &other) const {
      return solution == other.solution && rhs == other.rhs &&
             ...
             rows == other.rows && iterations == other.iterations &&
...
DeviceBiCGSTAB::DeviceBiCGSTAB(
  ...
  TI_ERROR_IF(!program_ || operator_handle.program() != program_ ||
              ...
              "Device BiCGSTAB requires a CUDA or Vulkan operator in its "
              "owning Program.");
  ...
  TI_ERROR_IF(descriptor.domain != descriptor.range ||
              ...
              "Device BiCGSTAB requires a non-empty square scalar f32 "
  ...
  if (stored_matrix_) {
    TI_ERROR_IF(stored_matrix_->num_rows() != rows_ ||
                ...
                "Device BiCGSTAB stored provider does not match its "
                "operator descriptor.");
  }
  validate_controls();
  operator_plan_ =
...
void DeviceBiCGSTAB::validate_controls() const {
  TI_ERROR_IF(max_iterations_ < 0 || !std::isfinite(absolute_tolerance_) ||
                  ...
                  (absolute_tolerance_ == 0.0f &&
                   relative_tolerance_ == 0.0f),
              "Device BiCGSTAB requires non-negative iterations and finite "
              "non-negative atol/rtol with at least one positive "
              "tolerance.");
}

void DeviceBiCGSTAB::allocate_workspace() {
  auto vector = [&] {
  ...
  try {
    residual_ = vector();
    shadow_residual_ = vector();
    direction_ = vector();
    operator_direction_ = vector();
    intermediate_ = vector();
    operator_intermediate_ = vector();
    if (has_preconditioner()) {
      preconditioned_direction_ = vector();
      preconditioned_intermediate_ = vector();
    }
...
void DeviceBiCGSTAB::release_workspace() {
  ...
  release(preconditioned_intermediate_);
  release(preconditioned_direction_);
  release(operator_intermediate_);
  release(intermediate_);
...
void DeviceBiCGSTAB::initialize_cuda() {
#if defined(TI_WITH_CUDA)
  auto &cublas = CUBLASDriver::get_instance();
  TI_ERROR_IF(!cublas.is_loaded() && !cublas.load_cublas(),
              "Device BiCGSTAB failed to load cuBLAS.");
  cublasHandle_t handle = nullptr;
  cublas.cubCreate(&handle);
  TI_ERROR_IF(!handle,
              "Device BiCGSTAB failed to create a cuBLAS handle.");
  ...
  TI_ERROR_IF(observed != nullptr,
              "Device BiCGSTAB could not bind cuBLAS to the solver stream.");
  ...
  TI_ERROR_IF(mode != CUBLAS_POINTER_MODE_DEVICE,
              "Device BiCGSTAB could not enable cuBLAS device scalar mode.");
...
void DeviceBiCGSTAB::release_cuda() {
#if defined(TI_WITH_CUDA)
  if (cublas_handle_) {
...
void DeviceBiCGSTAB::configure_execution_policy(
  ...
  TI_ERROR_IF(solve_calls_ != 0,
              "Device BiCGSTAB execution policy must be configured before "
              "solve.");
...
bool DeviceBiCGSTAB::has_preconditioner() const {
  return preconditioner_plan_ != nullptr;
}

bool DeviceBiCGSTAB::native_stored_provider() const {
...
std::uintptr_t DeviceBiCGSTAB::address(const Ndarray *array) const {
  return program_->get_ndarray_data_ptr_as_int(array);
}

void DeviceBiCGSTAB::apply_operator(
...
void DeviceBiCGSTAB::apply_preconditioner(
    const OperatorPinnedAction &generation,
    const Ndarray &input,
    const Ndarray &output) {
  TI_ERROR_IF(!preconditioner_plan_,
              "Device BiCGSTAB preconditioner plan is unavailable.");
...
void DeviceBiCGSTAB::backend_dot(const Ndarray &left,
                                 const Ndarray &right,
                                 const Ndarray &output,
                                 void *stream) {
  if (program_->compile_config().arch == Arch::cuda) {
...
void DeviceBiCGSTAB::backend_true_residual(
    const OperatorPinnedAction &generation,
    const Ndarray &x,
    const Ndarray &b,
    void *stream,
    bool native_capture) {
  apply_operator(generation, x, *operator_intermediate_, stream,
                 native_capture);
  if (program_->compile_config().arch == Arch::cuda) {
#if defined(TI_WITH_CUDA)
    cuda::driver_zero_strided(
        reinterpret_cast<void *>(address(intermediate_)), rows_,
    ...
    cuda::driver_add_scaled_strided(
        reinterpret_cast<void *>(address(operator_intermediate_)),
        reinterpret_cast<void *>(address(intermediate_)), rows_,
        cuda::CudaTransformValueType::f32, 0, sizeof(float), 0,
        sizeof(float), -1.0, stream);
#else
    ...
    program_->copy_ndarray_fast(intermediate_, const_cast<Ndarray *>(&b));
    program_->vulkan_sparse_axpy(operator_intermediate_, intermediate_, rows_,
                                 -1.0f);
  }
  backend_dot(*intermediate_, *intermediate_, *dot0_, stream);
}

void DeviceBiCGSTAB::backend_scalar_stage(int stage,
                                          bool limit_reached,
                                          void *stream) {
  if (program_->compile_config().arch == Arch::cuda) {
#if defined(TI_WITH_CUDA)
    cuda::driver_sparse_bicgstab_scalar_f32(
        ...
        relative_tolerance_, stage, limit_reached, stream);
#else
    TI_NOT_IMPLEMENTED;
#endif
    return;
  }
  program_->vulkan_sparse_bicgstab_scalar(
...
void DeviceBiCGSTAB::backend_direction(void *stream) {
  if (program_->compile_config().arch == Arch::cuda) {
#if defined(TI_WITH_CUDA)
    cuda::driver_sparse_bicgstab_direction_f32(
        reinterpret_cast<void *>(address(residual_)),
        reinterpret_cast<void *>(address(direction_)),
        reinterpret_cast<void *>(address(operator_direction_)),
  ...
  program_->vulkan_sparse_bicgstab_direction(
      residual_, direction_, operator_direction_, state_, rows_);
}

void DeviceBiCGSTAB::backend_intermediate(void *stream) {
  if (program_->compile_config().arch == Arch::cuda) {
#if defined(TI_WITH_CUDA)
    cuda::driver_sparse_bicgstab_intermediate_f32(
        reinterpret_cast<void *>(address(residual_)),
        reinterpret_cast<void *>(address(operator_direction_)),
        reinterpret_cast<void *>(address(intermediate_)),
        reinterpret_cast<void *>(address(state_)), rows_, stream);
#else
    TI_NOT_IMPLEMENTED;
#endif
  ...
  program_->vulkan_sparse_bicgstab_intermediate(
      residual_, operator_direction_, intermediate_, state_, rows_);
}

void DeviceBiCGSTAB::backend_commit(
    const Ndarray &x,
  ...
  if (program_->compile_config().arch == Arch::cuda) {
#if defined(TI_WITH_CUDA)
    cuda::driver_sparse_bicgstab_commit_f32(
        reinterpret_cast<void *>(address(&solution_direction)),
        reinterpret_cast<void *>(address(&solution_intermediate)),
        reinterpret_cast<void *>(address(intermediate_)),
        reinterpret_cast<void *>(address(operator_intermediate_)),
  ...
  program_->vulkan_sparse_bicgstab_commit(
      const_cast<Ndarray *>(&solution_direction),
      const_cast<Ndarray *>(&solution_intermediate), intermediate_,
      operator_intermediate_, const_cast<Ndarray *>(&x), residual_, state_,
      rows_);
}

void DeviceBiCGSTAB::backend_reconcile(const Ndarray &x, void *stream) {
  if (program_->compile_config().arch == Arch::cuda) {
#if defined(TI_WITH_CUDA)
    cuda::driver_sparse_bicgstab_reconcile_f32(
        reinterpret_cast<void *>(address(intermediate_)),
        reinterpret_cast<void *>(address(residual_)),
        reinterpret_cast<void *>(address(shadow_residual_)),
        reinterpret_cast<void *>(address(direction_)),
        reinterpret_cast<void *>(address(operator_direction_)),
        reinterpret_cast<void *>(address(&x)),
        reinterpret_cast<void *>(address(state_)), rows_, stream);
  ...
  program_->vulkan_sparse_bicgstab_reconcile(
      intermediate_, residual_, shadow_residual_, direction_,
      operator_direction_, const_cast<Ndarray *>(&x), state_, rows_);
}

void DeviceBiCGSTAB::issue_iteration(
    const OperatorPinnedAction &operator_generation,
    const OperatorPinnedAction &preconditioner_generation,
    const Ndarray &x,
    void *stream,
    bool native_capture) {
  backend_dot(*shadow_residual_, *residual_, *dot0_, stream);
  backend_scalar_stage(1, false, stream);
  backend_direction(stream);
  const Ndarray *solution_direction = direction_;
  if (has_preconditioner()) {
    apply_preconditioner(preconditioner_generation, *direction_,
                         *preconditioned_direction_);
    solution_direction = preconditioned_direction_;
  }
  apply_operator(operator_generation, *solution_direction,
                 *operator_direction_, stream, native_capture);
  backend_dot(*shadow_residual_, *operator_direction_, *dot0_, stream);
  backend_scalar_stage(2, false, stream);
  backend_intermediate(stream);
  backend_dot(*intermediate_, *intermediate_, *dot0_, stream);
  backend_scalar_stage(3, false, stream);
  const Ndarray *solution_intermediate = intermediate_;
  if (has_preconditioner()) {
    apply_preconditioner(preconditioner_generation, *intermediate_,
                         *preconditioned_intermediate_);
    solution_intermediate = preconditioned_intermediate_;
  }
  apply_operator(operator_generation, *solution_intermediate,
                 *operator_intermediate_, stream, native_capture);
  backend_dot(*operator_intermediate_, *intermediate_, *dot0_, stream);
  backend_dot(*operator_intermediate_, *operator_intermediate_, *dot1_,
              stream);
  backend_scalar_stage(4, false, stream);
  backend_commit(x, *solution_direction, *solution_intermediate, stream);
  backend_dot(*residual_, *residual_, *dot0_, stream);
  backend_scalar_stage(5, false, stream);
}

void DeviceBiCGSTAB::issue_chunk(
    const OperatorPinnedAction &operator_generation,
    const OperatorPinnedAction &preconditioner_generation,
    const Ndarray &x,
    const Ndarray &b,
  ...
  for (int i = 0; i < chunk_iterations; ++i) {
    issue_iteration(operator_generation, preconditioner_generation, x, stream,
                    native_capture);
  }
  backend_true_residual(operator_generation, x, b, stream, native_capture);
  backend_scalar_stage(6, limit_reached, stream);
  backend_reconcile(x, stream);
}

bool DeviceBiCGSTAB::try_submit_cuda_chunk(
    const OperatorPinnedAction &operator_generation,
    const OperatorPinnedAction &preconditioner_generation,
    const Ndarray &x,
    const Ndarray &b,
  ...
  if (!cuda_replay_) {
    cuda_replay_ = std::make_unique<DeviceBiCGSTABCudaReplayState>();
  }
  if (cuda_replay_->disabled) {
    return false;
  }
  DeviceBiCGSTABCudaReplayState::Key key;
  ...
  const Ndarray *arrays[] = {
      residual_, shadow_residual_, direction_, operator_direction_,
      intermediate_, operator_intermediate_, preconditioned_direction_,
      preconditioned_intermediate_, initial_residual_squared_, rhs_squared_,
      dot0_, dot1_, state_};
  for (std::size_t i = 0; i < std::size(arrays); ++i) {
    key.resources[i] = arrays[i] ? address(arrays[i]) : 0;
  }
  ...
  // Lazily initialize the vendor sparse-library path before capture.
  if (csr) {
    csr->spmv(address(direction_), address(operator_direction_), nullptr);
  } else {
    bsr->spmv(address(direction_), address(operator_direction_), nullptr);
  }
  ...
  const auto end_error = driver.stream_end_capture.call(capture_stream, &graph);
  cublas.cubSetStream(static_cast<cublasHandle_t>(cublas_handle_), nullptr);
...
bool DeviceBiCGSTAB::try_submit_vulkan_chunk(
    const OperatorPinnedAction &operator_generation,
    const OperatorPinnedAction &preconditioner_generation,
    const Ndarray &x,
    const Ndarray &b,
  ...
  if (!vulkan_replay_) {
    vulkan_replay_ =
        std::make_unique<DeviceBiCGSTABVulkanReplayState>();
  }
  ...
  push_bicgstab_stamp(key, operator_generation.resource_stamp());
  const Ndarray *resources[] = {
      &x, &b, residual_, shadow_residual_, direction_, operator_direction_,
      intermediate_, operator_intermediate_, preconditioned_direction_,
      preconditioned_intermediate_, initial_residual_squared_, rhs_squared_,
      dot0_, dot1_, state_};
  for (const auto *resource : resources) {
    push_bicgstab_resource(key, resource);
  }
  ...
  slot.operator_numeric_revision = operator_stamp.numeric_revision;
  const bool replaces = slot.cache.entry.cmdlist && slot.cache.entry.key != key;
...
void DeviceBiCGSTAB::read_state(bool synchronize) {
...
void DeviceBiCGSTAB::solve(Program *program,
                           const Ndarray &x,
                           const Ndarray &b) {
  TI_ERROR_IF(program != program_,
              "Device BiCGSTAB solve must use its construction Program.");
  auto check_vector = [&](const char *role, const Ndarray &array) {
    TI_ERROR_IF(array.shape.size() != 1 ||
                ...
                "Device BiCGSTAB {} must contain exactly {} scalar f32 "
                "entries.",
                role, rows_);
  };
  ...
  TI_ERROR_IF(x.get_device_allocation() == b.get_device_allocation(),
              "Device BiCGSTAB solution and RHS must not alias.");
  ...
  std::vector<const Ndarray *> resources = {
      &x, &b, residual_, shadow_residual_, direction_,
      operator_direction_, intermediate_, operator_intermediate_,
      initial_residual_squared_, rhs_squared_, dot0_, dot1_, state_};
  if (preconditioned_direction_) {
    resources.push_back(preconditioned_direction_);
    resources.push_back(preconditioned_intermediate_);
  }
  program_->retain_ndarrays_for_external_submission(resources.data(),
                                                     resources.size());
  ...
  backend_scalar_stage(0, max_iterations_ == 0, nullptr);
  backend_reconcile(x, nullptr);
  ...
  status_ = static_cast<SparseSolveStatus>(state_int(host_state_, kStatus));
  iterations_ = state_int(host_state_, kCompletedIterations);
  breakdown_reason_ =
      decode_breakdown_reason(state_int(host_state_, kBreakdownReason));
  ...
  const auto chunks = static_cast<std::uint64_t>(slot_index);
  const auto physical = static_cast<std::uint64_t>(executed);
  operator_apply_calls_ += 1u + 2u * physical + chunks;
  preconditioner_apply_calls_ +=
      has_preconditioner() ? 2u * physical : 0u;
  dot_product_calls_ += 2u + 6u * physical + chunks;
  vector_update_calls_ += 1u + 3u * physical + chunks;
  device_scalar_operations_ += 1u + 5u * physical + chunks;
  ...
  if (program_->compile_config().arch == Arch::vulkan) {
    device_to_device_bytes_ += (1u + chunks) * vector_bytes;
  }
}

SparseSolveResult DeviceBiCGSTAB::get_last_result() const {
...
DeviceBiCGSTAB::debug_runtime_statistics() const {
  std::lock_guard<std::mutex> lock(solve_mutex_);
  ...
  result.operator_numeric_changed_since_last_solve =
      solve_calls_ > 0 && stamp.numeric_revision != last_solve_numeric_version_;
  ...
  result.solver_chunk_direct_submissions = solver_chunk_direct_submissions_;
  ...
  const std::uint64_t vector_count = has_preconditioner() ? 8u : 6u;
  result.persistent_vector_count = vector_count;
  result.persistent_vector_reserved_bytes =
      vector_count * static_cast<std::uint64_t>(rows_) * sizeof(float);
  result.persistent_scalar_count = kStateWords + 4;
  result.persistent_scalar_reserved_bytes = kStateBytes + 4 * sizeof(float);
...
std::unique_ptr<DeviceBiCGSTAB> make_device_bicgstab_solver(
    ...
    float absolute_tolerance,
    float relative_tolerance) {
  return std::make_unique<DeviceBiCGSTAB>(
