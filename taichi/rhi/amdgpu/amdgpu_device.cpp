...
DeviceAllocation AmdgpuDevice::allocate_memory_runtime(
  ...
  if (params.host_read || params.host_write) {
    ...
    info.ptr =
        DeviceMemoryPool::get_instance(Arch::amdgpu).allocate_with_cache(this, params);
...
void AmdgpuDevice::dealloc_memory(DeviceAllocation handle) {
  ...
  if (info.use_cached) {
    DeviceMemoryPool::get_instance(Arch::amdgpu).release(info.size, (uint64_t *)info.ptr,
                                             false);
  } else if (!info.use_preallocated) {
    DeviceMemoryPool::get_instance(Arch::amdgpu).release(info.size, info.ptr);
  }
  info.ptr = nullptr;
}
