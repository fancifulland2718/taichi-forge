...
from taichi_forge.lang.enums import SNodeGradType
...
from taichi_forge.lang.kernel_arguments import SparseMatrixProxy
...
# created. Unfortunately, `root` exists in both taichi_forge.lang.impl module and
...
# gets delayed. `_root_fb` will only exist in the taichi_forge.lang.impl module, so
# writing to it is would result in less for maintenance cost.
#
# `_root_fb` will be overridden inside :func:`taichi_forge.lang.init`.
...
class _Root:
    ...
    @staticmethod
    def parent(n=1):
        """Same as :func:`taichi_forge.SNode.parent`"""
        return _root_fb.root.parent(n)

    @staticmethod
    def _loop_range():
        """Same as :func:`taichi_forge.SNode.loop_range`"""
        return _root_fb.root._loop_range()

    @staticmethod
    def _get_children():
        """Same as :func:`taichi_forge.SNode.get_children`"""
    ...
    @property
    def shape(self):
        """Same as :func:`taichi_forge.SNode.shape`"""
...
root = _Root()
"""Root of the declared Taichi :func:`~taichi_forge.lang.impl.field`s.
...
def field(dtype, *args, **kwargs):
    ...
    the complexity of how its underlying :class:`~taichi_forge.lang.snode.SNode` are
    actually defined. The data in a Taichi field can be directly accessed by
    a Taichi :func:`~taichi_forge.lang.kernel_impl.kernel`.
...
def zero(x):
    ...
    Args:
        x (Union[:mod:`~taichi_forge.types.primitive_types`, :class:`~taichi_forge.Matrix`]): The input.
...
def grouped(x):
    ...
    Args:
        x (:func:`~taichi_forge.ndrange`): an iterator object returned by `ti.ndrange`.
...
def stop_grad(x):
    ...
    Args:
        x (:class:`~taichi_forge.Field`): A field.
    """
