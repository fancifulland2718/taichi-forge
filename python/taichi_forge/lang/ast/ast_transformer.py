...
from taichi_forge.lang import _ndarray, any_array, expr, impl, kernel_arguments, matrix, mesh
from taichi_forge.lang import ops as ti_ops
from taichi_forge.lang._ndrange import _Ndrange, ndrange
from taichi_forge.lang.argpack import ArgPackType
from taichi_forge.lang.ast.ast_transformer_utils import Builder, LoopStatus, ReturnStatus
...
from taichi_forge.lang.matrix import Matrix, MatrixType, Vector
...
class ASTTransformer(Builder):
    ...
    def is_external_func(ctx, func) -> bool:
        ...
        if hasattr(func, "_is_taichi_function") or hasattr(func, "_is_wrapped_kernel"):  # taichi func/kernel
            return False
        if hasattr(func, "__module__") and func.__module__ and func.__module__.startswith("taichi_forge."):
            return False
        return True
        ...
        try:
            ...
            if ASTTransformer.is_external_func(ctx, node.func.ptr):
                ...
                if args_has_expr and (module == math or module == np):
                    exec_str = f"from taichi_forge import {func.__name__}"
                    try:
                        exec(exec_str, {})
                    except:
                        pass
    ...
    def build_Attribute(ctx, node):
        ...
        if isinstance(node.value.ptr, Expr) and not hasattr(node.value.ptr, node.attr):
            if node.attr in Matrix._swizzle_to_keygroup:
                ...
                from taichi_forge.lang import (  # pylint: disable=C0415
    ...
    def build_BinOp(ctx, node):
        ...
        from taichi_forge.lang.matrix_ops import matmul
