...
class SparseMINRES:
    """MINRES solver for explicit symmetric-indefinite SparseMatrix systems.

    The first provider is a Forge-owned, identity-preconditioned recurrence for
    mutable Eigen CSR/CSC matrices on CPU. ``A`` must be a complete explicit
    symmetric matrix; storage format and square shape do not prove symmetry.
    Fixed-pattern and GPU matrices are rejected without a host fallback.

    Convergence requires
    ``||b - A x||_2 <= max(atol, rtol * ||b||_2)``. Both solution and true
    residual must be finite before a solve can report convergence.
    """
    ...
    def __init__(
        ...
        A._require_operation("public_minres")
        self.dtype = A.dtype
        ...
        if self.ti_arch not in (_ti_core.Arch.x64, _ti_core.Arch.arm64):
            raise TaichiRuntimeError(
                f"Unsupported SparseMINRES arch: {self.ti_arch}"
            )
        if self.dtype == f32:
            if self._fixed_cpu_native:
                self.solver = (
                    _ti_core._make_float_cpu_fixed_sparse_minres_solver(
                        get_runtime().prog,
                        A.matrix,
                        max_iter,
                        atol,
                ...
                self.solver = _ti_core.make_float_sparse_minres_solver(
                    A.matrix, max_iter, atol, True, rtol
                )
        elif self.dtype == f64:
            if self._fixed_cpu_native:
                self.solver = (
                    _ti_core._make_double_cpu_fixed_sparse_minres_solver(
                        get_runtime().prog,
                        A.matrix,
                        max_iter,
                        atol,
                ...
                self.solver = _ti_core.make_double_sparse_minres_solver(
                    A.matrix, max_iter, atol, True, rtol
                )
        else:
            raise TaichiRuntimeError(
                f"Unsupported SparseMINRES dtype: {self.dtype}"
            )
    ...
    def _validate_vector(self, value, role, expected_size, allow_none=False):
        ...
        if isinstance(value, ScalarNdarray):
            if value.arr is None or value._runtime_prog is not get_runtime().prog:
                raise TaichiRuntimeError(
                    f"SparseMINRES {role} cannot be used after its Taichi "
                    "runtime has been reset"
                )
            if value.dtype != self.dtype or value.shape != (expected_size,):
                raise TaichiRuntimeError(
                    f"SparseMINRES {role} must be a scalar {self.dtype} "
        ...
        if isinstance(value, np.ndarray):
            if value.shape != (expected_size,):
                raise TaichiRuntimeError(
                    f"SparseMINRES {role} must have shape "
                    f"({expected_size},), got {value.shape}"
                )
            if not np.issubdtype(value.dtype, np.floating):
                raise TaichiRuntimeError(
                    f"SparseMINRES {role} must have a floating dtype, "
                    f"got {value.dtype}"
                )
            return value
        raise TaichiRuntimeError(
            f"Unsupported SparseMINRES {role} type: {type(value)}"
        )
    ...
    def _debug_runtime_stats(self):
        """Returns private recurrence resource and operation telemetry."""
        self.matrix._ensure_valid()
