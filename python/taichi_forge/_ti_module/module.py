...
from taichi_forge._ti_module.cppgen import generate_header
from taichi_forge.aot._export import _aot_kernels
...
from taichi_forge.types.primitive_types import integer_type_ids, real_type_ids
        ...
        if record.template_types:
            ...
            for k, v in record.template_types.items():
                ...
                if isinstance(v, int) or id(v) in integer_type_ids:
                    ...
                    value = taichi_forge.ndarray(v.dtype, (1,) * v.ndim)
                elif isinstance(v, TextureType):
                    value = taichi_forge.Texture(taichi_forge.Format.rgba8, (4,) * v.num_dimensions)
                elif isinstance(v, RWTextureType):
                    value = taichi_forge.Texture(v.fmt, (4,) * v.num_dimensions)
                else:
