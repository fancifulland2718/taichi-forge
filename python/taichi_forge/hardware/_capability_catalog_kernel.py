...
def kernel_intrinsic_operations(_operation):
    return (
        ...
        _operation(
            ...
            update_policy="immutable",
            dtypes=(
                "sampled normalized:u8/u16",
                "sampled:f16/f32",
            ),
            shapes_or_tiles=("1D", "2D", "3D"),
            layouts=("cuda_array",),
            numeric_contracts=(
                "sample_lod:single_mip_normalized_coords_vec4_f32",
                "fetch:integer_texel_coords_vec4_f32",
            ),
            deterministic=False,
            requirements=(
                "NVIDIA CUDA Driver array and texture-object symbols",
                "one, two, or four channels",
                "matching min and mag filters",
                "JIT CUDA kernel execution",
            ),
            public_api="ti.Texture(..., sampler=ti.hardware.sampling.SamplerConfig(...))",
            notes=(
                "Driver API symbols are loaded from the installed NVIDIA driver; the wheel has no CUDART or Toolkit dependency.",
                ...
                "CUDA Graph capture remains unsupported until texture generation retention and rebinding are modeled.",
