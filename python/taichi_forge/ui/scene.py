...
from taichi_forge.lang.ops import atomic_add
...
class SceneV2:
    ...
    get_scene() function of a taichi_forge.ui.Window.
    """
    ...
    def set_camera(self, camera):
        ...
        Args:
            camera (:class:`~taichi_forge.ui.Camera`): A camera instance.
        """
    ...
    def lines(
        ...
        if isinstance(vertices, taichi_forge.Field):
            copy_all_to_vbo(vbo, vertices, 0, 0, per_vertex_color if has_per_vertex_color else 0)
        else:
            copy_to_vbo_vector(vbo, vertices, 0, 3, taichi_forge.Vector([0, 0, 0]))
            if has_per_vertex_color:
                copy_to_vbo_vector(vbo, per_vertex_color, 8, 4, taichi_forge.Vector([1, 1, 1, 1]))
    ...
    def particles(
        ...
        if isinstance(centers, taichi_forge.Field):
            copy_all_to_vbo_particle(
                vbo,
                centers,
            ...
            copy_to_vbo_vector(vbo, centers, 0, 3, taichi_forge.Vector([0, 0, 0]))
    ...
    def point_light(self, pos, color):  # pylint: disable=W0235
        ...
        Args:
            pos (list, tuple, :class:`~taichi_forge.types.vector(3, float)`):
                3D vector for light position.
            color (list, tuple, :class:`~taichi_forge.types.vector(3, float)`):
...
class Scene:
    ...
    def __init__(self):
        ...
        warnings.warn(
            "Instantiating ti.ui.Scene directly is deprecated, use the get_scene() function from a taichi_forge.ui.Window object instead.",
            DeprecationWarning,
        )
