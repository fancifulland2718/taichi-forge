...
from benchmarks.qualification.audit_single_kernel_run import (
    ...
    _marching_squares_kernel_control_route_isolated as _audit_marching_squares_kernel_control_route_isolated,
    _bfs_worklist_kernel_control_route_isolated as _audit_bfs_worklist_kernel_control_route_isolated,
    _bfs_worklist_native_route_admitted as _audit_bfs_worklist_native_route_admitted,
    _falling_sand_kernel_control_route_isolated as _audit_falling_sand_kernel_control_route_isolated,
    _falling_sand_native_route_admitted as _audit_falling_sand_native_route_admitted,
    _sparse_block_stencil_route_isolated as _audit_sparse_block_stencil_route_isolated,
)
...
class SingleKernelMicrobenchTest(unittest.TestCase):
    ...
    def test_sparse_block_stencil_route_and_endpoint_are_fail_closed(self):
        ...
        validation = {
            ...
            "expected_endpoint_fingerprint": json.loads(json.dumps(fingerprint)),
        }
        left = {
            "operation": "sparse_block_stencil",
        ...
        self.assertTrue(_endpoint_equivalent({"forge": left, "vanilla": right}, "forge", "vanilla"))
        self.assertTrue(_audit_endpoint_equivalent(left, right))
        right["validation_after"]["endpoint_fingerprint"]["sha256"] = "c" * 64
        self.assertFalse(_endpoint_equivalent({"forge": left, "vanilla": right}, "forge", "vanilla"))
    ...
    def test_ordinary_kernel_route_proves_benchmark_owned_single_launch(self):
        ...
        self.assertEqual(route["classification"], "forge_ordinary_taichi_kernel")
    ...
    def test_ordinary_endpoint_equivalence_uses_actual_fingerprints(self):
        ...
        self.assertTrue(_audit_endpoint_equivalent(results["forge"], results["vanilla"]))
        results["vanilla"]["validation_after"]["endpoint_fingerprint"]["sample_values"][2] = 3.5
        results["vanilla"]["validation_after"]["endpoint_fingerprint"]["sum"] = 10.5
        self.assertFalse(_endpoint_equivalent(results, "forge", "vanilla"))
        self.assertFalse(_audit_endpoint_equivalent(results["forge"], results["vanilla"]))
    ...
    def test_native_reduce_endpoint_equivalence_uses_exact_actual_values(self):
        ...
        self.assertTrue(_endpoint_equivalent(results, "forge", "forge_kernel"))
        self.assertTrue(_audit_endpoint_equivalent(results["forge"], results["forge_kernel"]))
        results["forge_kernel"]["validation_after"]["actual"] = -7
        self.assertFalse(_endpoint_equivalent(results, "forge", "forge_kernel"))
        self.assertFalse(_audit_endpoint_equivalent(results["forge"], results["forge_kernel"]))
    ...
    def test_native_compact_endpoint_equivalence_uses_exact_ordered_fingerprint(self):
        validation = {
            "passed": True,
    ...
    def test_device_prefix_endpoint_equivalence_uses_both_exact_outputs(self):
        ...
        self.assertTrue(_endpoint_equivalent(results, "forge", "forge_kernel"))
        self.assertTrue(_audit_endpoint_equivalent(results["forge"], results["forge_kernel"]))
        results["forge_kernel"]["validation_after"]["scanned"]["actual_samples"][1] = 99
        self.assertFalse(_endpoint_equivalent(results, "forge", "forge_kernel"))
        self.assertFalse(_audit_endpoint_equivalent(results["forge"], results["forge_kernel"]))

    def test_profiler_range_rejects_normal_parent_or_non_cuda_run(self):
        with self.assertRaisesRegex(ValueError, "requires one CUDA score sample in child mode"):
            main(
                [
                    "--operation",
                    "fill",
    ...
    def test_balanced_pair_orders_are_adjacent_and_alternating(self):
        ...
        self.assertTrue(all(set(order) == {"forge", "vanilla"} for order in orders))
    ...
    def test_forge_native_control_uses_two_routes_from_forge(self):
        participants = comparison_participants("forge-native-vs-forge-kernel", "native_transform")
        ...
        definition = comparison_definition("forge-native-vs-forge-kernel", "native_transform")
        self.assertEqual(definition["speedup_formula"], "forge_kernel_ms / forge_ms")
    ...
    def test_forge_native_control_rejects_non_thin_operation(self):
        with self.assertRaisesRegex(ValueError, "thin/native operations"):
            comparison_participants("forge-native-vs-forge-kernel", "mpm_graph")

    def test_kernel_compatibility_axis_uses_same_adapter_across_packages(self):
        definition = comparison_definition("forge-kernel-vs-vanilla", "native_reduce")
        self.assertEqual(
            (definition["subject"], definition["baseline"]),
            ("forge_kernel", "vanilla_kernel"),
        )
        self.assertEqual(definition["speedup_formula"], "vanilla_kernel_ms / forge_kernel_ms")
        self.assertIn("same vanilla-compatible kernel", definition["attribution"])

    def test_forge_kernel_route_is_not_labeled_vanilla(self):
        source_sha256 = "a" * 64
        route = _native_transform_route(None, "forge_kernel", "cuda", source_sha256)
        self.assertTrue(route["passed"])
        self.assertEqual(
        ...
        self.assertEqual(route["ti_kernel_invocations_per_replay"], 1)
        self.assertFalse(route["physical_backend_launches_assumed"])
        self.assertFalse(_native_transform_route(object(), "forge_kernel", "cuda", source_sha256)["passed"])
        self.assertFalse(_native_transform_route(None, "forge_kernel", "cuda", "short")["passed"])

    def test_native_reduce_kernel_route_proves_benchmark_control(self):
        source_sha256 = "a" * 64
        route = _native_reduce_route(None, "forge_kernel", "cuda", source_sha256)
        self.assertTrue(route["passed"])
        self.assertEqual(route["adapter"], "benchmark_defined_ti_kernel")
        self.assertEqual(route["kernel_source_owner"], "benchmark")
        self.assertEqual(route["kernel_source_sha256"], source_sha256)
        ...
        self.assertEqual(route["ti_kernel_invocations_per_replay"], 1)
        self.assertFalse(route["physical_backend_launches_assumed"])
        self.assertFalse(_native_reduce_route(object(), "forge_kernel", "cuda", source_sha256)["passed"])
        self.assertFalse(_native_reduce_route(None, "forge_kernel", "cuda", "short")["passed"])
    ...
    def test_pilot_confirms_candidate_batch_after_steady_state(self):
        ...
        with patch(
            "benchmarks.qualification.single_kernel_microbench._timed_batch", side_effect=lambda *_: next(timings)
        ):
            batch, attempts = _calibrate_batch(None, lambda: None, 120.0)
        self.assertEqual(batch, 240)
        self.assertEqual([row["batch_size"] for row in attempts], [1, 120, 120, 120, 240, 240, 240])
        self.assertEqual(
    ...
    def test_qualification_minimums_are_encoded_once(self):
        ...
        self.assertEqual(
            qualification_policy_errors(args), ["qualification pairs must be even for exact AB/BA balance"]
        )
    ...
    def test_external_python_override_is_diagnostic_only_and_fail_closed(self):
        ...
        args = argparse.Namespace(intent="diagnostic", allow_external_python=True)
        ...
        args.intent = "diagnostic"
        observation["reasons"].append("CPU utilization 30.0% exceeds 20.0%")
        self.assertFalse(_noise_allows_diagnostic_execution(args, observation))
    ...
    def test_gpu_uuid_normalization_matches_runtime_and_nvidia_forms(self):
        ...
        self.assertEqual(
            normalize_gpu_uuid(bytes.fromhex("a69ec1380e308d1ee2994a0f2a2a6645")),
            "a69ec1380e308d1ee2994a0f2a2a6645",
        )
    ...
    def test_enhanced_memory_plateau_rejects_live_growth(self):
        before = {
            ...
            "pools": {
                ...
                "device": {"cached_blocks": 0, "cached_bytes": 0, "raw_bytes": 128, "raw_chunks": 1},
            },
        }
        ...
        self.assertIn("live_resources", result["runtime_memory"]["growing_fields"])
    ...
    def test_snode_lifecycle_initial_capacity_growth_can_be_allowed(self):
        ...
        after = json.loads(json.dumps(before))
        after["runtime_directory"].update(capacity=128, reserved_bytes=1024, growth_events=2)
        after["kernel_lifecycle"]["registered_executables"] = 3
        self.assertFalse(_snode_lifecycle_plateau(before, after)["passed"])
        self.assertTrue(
            _snode_lifecycle_plateau(
                before, after, require_directory_plateau=False, require_registration_plateau=False
            )["passed"]
        )
    ...
    def test_stability_failure_preserves_partial_replay_and_memory_evidence(self):
        ...
        with (
            patch(
                "benchmarks.qualification.single_kernel_microbench." "runtime_memory_observation",
                return_value=observation,
            ),
            patch("benchmarks.qualification.single_kernel_microbench." "working_set_bytes", side_effect=[100, 130]),
            patch(
                "benchmarks.qualification.single_kernel_microbench." "process_gpu_memory_mib", side_effect=[10.0, 25.0]
            ),
        ):
            with self.assertRaises(_StabilityReplayError) as captured:
                _run_stability(FakeTi(), launch, 10, 10, True, "vanilla")
    ...
    def test_native_reduce_route_requires_the_declared_cuda_plan(self):
        ...
        passed = _native_reduce_route(Workspace(), "forge", "cuda", "a" * 64)
        self.assertTrue(passed["passed"])
        self.assertEqual(passed["observed_method"], "cuda_device_reduce_ndarray")
        Plan.method_name = "unexpected_fallback"
        failed = _native_reduce_route(Workspace(), "forge", "cuda", "a" * 64)
    ...
    def test_native_transform_route_rejects_fallback(self):
        ...
        self.assertTrue(_native_transform_route(Workspace(), "forge", "cuda", "a" * 64)["passed"])
        Plan.backend = "field_kernel"
        self.assertFalse(_native_transform_route(Workspace(), "forge", "cuda", "a" * 64)["passed"])
    ...
    def test_native_gather_route_requires_cached_native_plan(self):
        ...
        route = _native_indexed_copy_route(Workspace(), "forge", "cuda", scatter=False, kernel_source_sha256="a" * 64)
        self.assertTrue(route["passed"])
        self.assertEqual(route["observed_method"], "cuda_device_gather_ndarray")
        Plan.method_name = "gather_i32_ndarray_kernel_fallback"
        self.assertFalse(
            _native_indexed_copy_route(Workspace(), "forge", "cuda", scatter=False, kernel_source_sha256="a" * 64)[
                "passed"
            ]
        )
    ...
    def test_native_gather_kernel_route_proves_benchmark_control(self):
        ...
        route = _native_indexed_copy_route(
            None, "forge_kernel", "cuda", scatter=False, kernel_source_sha256=source_sha256
        )
        self.assertTrue(route["passed"])
        self.assertEqual(route["adapter"], "benchmark_defined_ti_kernel")
        self.assertEqual(route["kernel_source_owner"], "benchmark")
        ...
        self.assertFalse(
            _native_indexed_copy_route(
                object(), "forge_kernel", "cuda", scatter=False, kernel_source_sha256=source_sha256
            )["passed"]
        )
        self.assertFalse(
            _native_indexed_copy_route(None, "forge_kernel", "cuda", scatter=False, kernel_source_sha256="short")[
                "passed"
            ]
        )
    ...
    def test_native_scatter_route_distinguishes_gather_plan(self):
        ...
        self.assertTrue(
            _native_indexed_copy_route(Workspace(), "forge", "cuda", scatter=True, kernel_source_sha256="a" * 64)[
                "passed"
            ]
        )
        self.assertFalse(
    ...
    def test_native_scatter_kernel_route_proves_benchmark_control(self):
        source_sha256 = "b" * 64
        route = _native_indexed_copy_route(
            None, "vanilla_kernel", "cuda", scatter=True, kernel_source_sha256=source_sha256
        )
        self.assertTrue(route["passed"])
        self.assertEqual(route["adapter"], "benchmark_defined_ti_kernel")
        self.assertEqual(route["kernel_source_owner"], "benchmark")
        ...
        self.assertFalse(
            _native_indexed_copy_route(
                object(), "vanilla_kernel", "cuda", scatter=True, kernel_source_sha256=source_sha256
            )["passed"]
        )
        self.assertFalse(
            _native_indexed_copy_route(None, "vanilla_kernel", "cuda", scatter=True, kernel_source_sha256="short")[
                "passed"
            ]
        )
    ...
    def test_native_compact_route_requires_cuda_device_plan(self):
        ...
        self.assertTrue(_native_compact_route(Workspace(), "forge", "cuda")["passed"])
        Plan.backend = "cuda_cub"
        self.assertFalse(_native_compact_route(Workspace(), "forge", "cuda")["passed"])

    def test_native_compact_kernel_route_proves_benchmark_pipeline(self):
        source_sha256 = "c" * 64
        route = _native_compact_route(None, "vanilla_kernel", "cuda", source_sha256, 65536)
        self.assertTrue(route["passed"])
        self.assertEqual(route["adapter"], "benchmark_defined_ti_kernel_pipeline")
        self.assertEqual(route["kernel_source_owner"], "benchmark")
        self.assertEqual(route["kernel_source_sha256"], source_sha256)
        self.assertFalse(route["helper_api_used"])
        ...
        self.assertFalse(route["physical_backend_launches_assumed"])
        self.assertFalse(_native_compact_route(object(), "vanilla_kernel", "cuda", source_sha256, 65536)["passed"])
        self.assertFalse(_native_compact_route(None, "vanilla_kernel", "cuda", "short", 65536)["passed"])
    ...
    def test_device_prefix_route_requires_both_native_stages(self):
        ...
        self.assertTrue(_device_prefix_chain_route(Workspace(), "forge", "cuda", 64)["passed"])
        ScanPlan.method_name = "legacy_scan"
        self.assertFalse(_device_prefix_chain_route(Workspace(), "forge", "cuda", 64)["passed"])

    def test_device_prefix_kernel_route_proves_benchmark_pipeline(self):
        source_sha256 = "d" * 64
        route = _device_prefix_chain_route(None, "forge_kernel", "cuda", 65536, source_sha256)
        self.assertTrue(route["passed"])
        self.assertEqual(route["adapter"], "benchmark_defined_ti_kernel_pipeline")
        self.assertEqual(route["kernel_source_owner"], "benchmark")
        self.assertEqual(route["kernel_source_sha256"], source_sha256)
        ...
        self.assertEqual(route["final_scan_copy_kernel_invocations_per_pipeline"], 0)
        ...
        self.assertFalse(route["physical_backend_launches_assumed"])
        self.assertFalse(_device_prefix_chain_route(object(), "forge_kernel", "cuda", 65536, source_sha256)["passed"])
        self.assertFalse(_device_prefix_chain_route(None, "vanilla_kernel", "cuda", 65536, "short")["passed"])
    ...
    def test_active_grid_kernel_route_proves_shared_graph_pipeline(self):
        ...
        source_sha256 = "e" * 64
        route = _active_grid_mpm_route(ForgeGraph(), "forge_kernel", "cuda", source_sha256, 65536, 128, None, None)
        self.assertTrue(route["passed"])
        self.assertEqual(route["adapter"], "benchmark_defined_ti_kernel_graph_pipeline")
        self.assertEqual(route["kernel_source_owner"], "benchmark")
        self.assertEqual(route["kernel_source_sha256"], source_sha256)
        self.assertFalse(route["helper_api_used"])
        self.assertFalse(route["specialized_api_used"])
        self.assertEqual(route["benchmark_workspace_field_count"], 0)
        self.assertEqual(route["graph_kernel_names"], ["reset_grid", "p2g", "update_grid_full", "g2p"])
        self.assertEqual(route["graph_dispatches_per_replay"], 4)
        self.assertEqual(route["ti_kernel_invocations_per_replay"], 4)
        self.assertFalse(route["physical_backend_launches_assumed"])
        self.assertFalse(
            _active_grid_mpm_route(ForgeGraph(), "forge_kernel", "cuda", "short", 65536, 128, None, None)["passed"]
        )
        self.assertFalse(
            _active_grid_mpm_route(ForgeGraph(), "forge_kernel", "cuda", source_sha256, 65536, 128, object(), None)[
                "passed"
            ]
        )
    ...
    def test_active_grid_endpoint_requires_exact_active_mask_evidence(self):
        ...
        self.assertTrue(_endpoint_equivalent(results, "forge", "forge_kernel"))
        self.assertTrue(_audit_endpoint_equivalent(results["forge"], results["forge_kernel"]))
        results["forge_kernel"]["validation_after"]["active_flags_sha256"] = "0" * 64
        self.assertFalse(_endpoint_equivalent(results, "forge", "forge_kernel"))
        self.assertFalse(_audit_endpoint_equivalent(results["forge"], results["forge_kernel"]))
    ...
    def test_particle_hash_route_requires_native_bucket_plan(self):
        ...
        route = _particle_hash_route(Workspace(), "forge", "cuda", bins=16)
        self.assertTrue(route["passed"])
        Plan.method_name = "field_kernel_fallback"
        self.assertFalse(_particle_hash_route(Workspace(), "forge", "cuda", bins=16)["passed"])

    def test_particle_hash_kernel_route_proves_shared_pipeline(self):
        source_sha256 = "9" * 64
        route = _particle_hash_route(None, "forge_kernel", "cuda", bins=16384, kernel_source_sha256=source_sha256)
        self.assertTrue(route["passed"])
        self.assertEqual(route["adapter"], "benchmark_defined_ti_kernel_pipeline")
        self.assertEqual(route["kernel_source_owner"], "benchmark")
        self.assertEqual(route["kernel_source_sha256"], source_sha256)
        ...
        self.assertEqual(route["non_scan_ti_kernel_invocations_per_replay"], 5)
        self.assertEqual(route["ti_kernel_invocations_per_replay"], 21)
        self.assertFalse(route["physical_backend_launches_assumed"])
        self.assertFalse(
            _particle_hash_route(object(), "forge_kernel", "cuda", bins=16384, kernel_source_sha256=source_sha256)[
                "passed"
            ]
        )
        self.assertFalse(
            _particle_hash_route(None, "vanilla_kernel", "cuda", bins=16384, kernel_source_sha256="short")["passed"]
        )
    ...
    def test_particle_hash_endpoint_requires_all_exact_vectors(self):
        ...
        self.assertTrue(_endpoint_equivalent(results, "forge", "forge_kernel"))
        self.assertTrue(_audit_endpoint_equivalent(results["forge"], results["forge_kernel"]))
        results["forge_kernel"]["validation_after"]["endpoint_vectors"]["canonical_output"]["actual_sha256"] = "e" * 64
        self.assertFalse(_endpoint_equivalent(results, "forge", "forge_kernel"))
        self.assertFalse(_audit_endpoint_equivalent(results["forge"], results["forge_kernel"]))
    ...
    def test_adaptive_pbd_route_requires_fixed_native_worklist(self):
        ...
        self.assertTrue(_adaptive_pbd_route(Worklist(), "forge", "cuda", 32, 10, "a" * 64)["passed"])
        Plan.method_name = "kernel_fallback"
        self.assertFalse(_adaptive_pbd_route(Worklist(), "forge", "cuda", 32, 10, "a" * 64)["passed"])

    def test_adaptive_pbd_kernel_route_is_strongly_admitted(self):
        route = _adaptive_pbd_route(None, "forge_kernel", "cuda", 65536, 10, "a" * 64)
        self.assertTrue(route["passed"])
        self.assertEqual(route["adapter"], "benchmark_defined_ti_kernel_pipeline")
        ...
        self.assertFalse(_adaptive_pbd_route(None, "vanilla_kernel", "cuda", 65536, 10, None)["passed"])
    ...
    def test_adaptive_pbd_endpoint_requires_full_vectors(self):
        ...
        self.assertTrue(_endpoint_equivalent(results, "forge", "forge_kernel"))
        self.assertTrue(_audit_endpoint_equivalent(results["forge"], results["forge_kernel"]))
        results["forge_kernel"]["validation_after"]["endpoint_vectors"]["positions_f32"]["sha256"] = "e" * 64
        self.assertFalse(_endpoint_equivalent(results, "forge", "forge_kernel"))
        self.assertFalse(_audit_endpoint_equivalent(results["forge"], results["forge_kernel"]))
    ...
    def test_falling_sand_route_admits_three_explicit_paths(self):
        ...
        source_sha256 = "c" * 64
        native = _falling_sand_route(Worklist(), "forge", "cuda", 32, 6, 3, 6, 3, source_sha256)
        ...
        control = _falling_sand_route(None, "forge_kernel", "cuda", 32, 6, 3, 6, 3, source_sha256)
        control_contract = {
            **contract,
        ...
        self.assertTrue(_falling_sand_kernel_control_route_isolated(control_child))
        self.assertTrue(_audit_falling_sand_kernel_control_route_isolated(control_child))
        control["helper_api_used"] = True
        self.assertFalse(_falling_sand_kernel_control_route_isolated(control_child))
        self.assertFalse(_audit_falling_sand_kernel_control_route_isolated(control_child))
    ...
    def test_falling_sand_endpoint_recomputes_all_raw_vectors(self):
        def exact(values):
            ...
            payload = b"".join(int(value).to_bytes(4, "little", signed=True) for value in values)
            ...
            return {
                ...
                "expected_samples": [values[index] for index in sample_indices],
        ...
        self.assertTrue(_endpoint_equivalent(results, "forge", "forge_kernel"))
        self.assertTrue(_audit_endpoint_equivalent(results["forge"], results["forge_kernel"]))
        results["forge_kernel"]["validation_after"]["endpoint_vectors"]["grid_i32"]["actual_values_i32"][1] = 99
        self.assertFalse(_endpoint_equivalent(results, "forge", "forge_kernel"))
        self.assertFalse(_audit_endpoint_equivalent(results["forge"], results["forge_kernel"]))
    ...
    def test_marching_squares_route_admits_native_and_strong_kernel_paths(self):
        ...
        native = _marching_squares_route(Workspace(), "forge", "cuda", 65536, "a" * 64)
        self.assertTrue(native["passed"])
        self.assertTrue(native["compact_route"]["passed"])
        self.assertEqual(native["stage_kernel_names"], ["classify", "stable_compact", "emit_cases"])

        control = _marching_squares_route(None, "forge_kernel", "cuda", 65536, "b" * 64)
        self.assertTrue(control["passed"])
        self.assertEqual(control["adapter"], "benchmark_defined_ti_kernel_pipeline")
        ...
        self.assertEqual(control["final_scan_copy_kernel_invocations"], 0)
        self.assertEqual(control["non_scan_ti_kernel_invocations_per_replay"], 4)
        self.assertEqual(control["ti_kernel_invocations_per_replay"], 20)
        self.assertFalse(control["physical_backend_launches_assumed"])
        self.assertFalse(_marching_squares_route(object(), "vanilla_kernel", "cuda", 65536, "b" * 64)["passed"])
        self.assertFalse(_marching_squares_route(None, "vanilla_kernel", "cuda", 65536, "short")["passed"])
        ...
        contract = {
            ...
            "kernel_stage_names": ["classify", "stage_flags", "scan_ping_pong", "stable_scatter", "emit_cases"],
        ...
        self.assertTrue(_marching_squares_kernel_control_route_isolated(child))
        self.assertTrue(_audit_marching_squares_kernel_control_route_isolated(child))
        del contract["kernel_scan_steps"]
        self.assertFalse(_marching_squares_kernel_control_route_isolated(child))
        self.assertFalse(_audit_marching_squares_kernel_control_route_isolated(child))
    ...
    def test_marching_squares_endpoint_requires_full_ordered_vectors(self):
        ...
        self.assertTrue(_endpoint_equivalent(results, "forge", "forge_kernel"))
        self.assertTrue(_audit_endpoint_equivalent(results["forge"], results["forge_kernel"]))
        results["forge_kernel"]["validation_after"]["endpoint_vectors"]["case_codes_i32"]["actual_sha256"] = "e" * 64
        self.assertFalse(_endpoint_equivalent(results, "forge", "forge_kernel"))
        self.assertFalse(_audit_endpoint_equivalent(results["forge"], results["forge_kernel"]))
    ...
    def test_bfs_route_admits_native_and_strong_kernel_paths(self):
        ...
        native = _bfs_worklist_route(Worklist(), "forge", "cuda", 32, 4, 4, "a" * 64)
        self.assertTrue(native["passed"])
        self.assertEqual(native["adapter"], "forge_native_device_worklist_frontier_pipeline")
        native_child = {
        ...
        control = _bfs_worklist_route(None, "forge_kernel", "cuda", 65536, 64, 256, "b" * 64)
        self.assertTrue(control["passed"])
        self.assertEqual(control["adapter"], "benchmark_defined_ti_kernel_pipeline")
        self.assertEqual(control["benchmark_workspace_field_count"], 4)
        self.assertEqual(control["ti_kernel_invocations_per_replay"], 194)
        self.assertFalse(control["physical_backend_launches_assumed"])
        self.assertFalse(_bfs_worklist_route(object(), "vanilla_kernel", "cuda", 65536, 64, 256, "b" * 64)["passed"])
        ...
        self.assertTrue(_audit_bfs_worklist_kernel_control_route_isolated(child))
        del contract["kernel_ti_invocations_per_replay"]
        self.assertFalse(_bfs_worklist_kernel_control_route_isolated(child))
        self.assertFalse(_audit_bfs_worklist_kernel_control_route_isolated(child))
    ...
    def test_bfs_endpoint_recomputes_full_raw_vectors(self):
        ...
        self.assertTrue(_endpoint_equivalent(results, "forge", "forge_kernel"))
        self.assertTrue(_audit_endpoint_equivalent(results["forge"], results["forge_kernel"]))
        results["forge_kernel"]["validation_after"]["endpoint_vectors"]["distance_i32"]["actual_values_i32"][2] = 99
        self.assertFalse(_endpoint_equivalent(results, "forge", "forge_kernel"))
        self.assertFalse(_audit_endpoint_equivalent(results["forge"], results["forge_kernel"]))
