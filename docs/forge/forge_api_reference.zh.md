...
| `plan.submit(rhs, initial_guess=None, out=None, pacer=None, lane=None, on_saturation="wait", telemetry=False, workspace_lane=None)` | 提交一次完整 recordable f32 CG/PCG solve 并返回 `SolvePlanSubmission`。 | 复用缓存的单 action Graph 与单个 backend ticket；`wait()` 不读 terminal，`result()` 只物化一次；配置的 workspace lane 各自拥有独立 Krylov storage。 |
...
| `operator.statistics()` / `plan.statistics()` | 返回 provider/plan execution 与 workspace 诊断。 | 单系统 GPU plan 在可用时报告 A/M、dot product、vector update work、logical/executed/masked iteration、workspace bytes、action counter、preconditioning side 与 chunk activity。recordable Graph Krylov 还报告两级 reduction 策略、block size、每线程 item 数、partial 数量、fixed scratch bytes 与 encoded window size；batch plan schema v4 另行报告 plan-owned recurrence Graph 活动，并明确 A/M provider action 不属于该 replay 范围。diagnostic snapshot 不属于数值结果。 |
