...
| `ti.linalg.experimental.vector_io_capabilities()` / `view.metadata` | 查询版本化 storage、layout、execution mode 与 indexed topology 合同。 | ndarray 为 direct；受支持 field/view 为 device-staged；不把 device staging 声明为 zero-copy。 |
| `operator.apply(x, out=None, *, alpha=1, beta=0, addend=None)` / `operator @ x` | 同步执行 `out = alpha * A(x) + beta * addend`。 | 一维 scalar ndarray 或受支持 field/view；CPU 支持通用系数；CUDA/Vulkan 支持 overwrite；`beta=0` 不读取 addend；禁止 input/output alias。 |
