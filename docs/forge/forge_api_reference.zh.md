...
| `operator.scaled(...)`、`.shifted(...)`、`operator + other`、`.compose(...)`、`.adjoint()`、`block_diagonal(...)`、`identity(...)` | 构造最小线性算子代数。 | CPU `f32/f64`；CUDA/Vulkan 支持 Program-bound f32 scale/shift/sum/compose。recordable shift 只增加一个 in-place `axpby`，不分配 identity-sized temporary。adjoint 需要显式 capability。 |
| `ti.linalg.inverse_block_diagonal(inverse_blocks, block_size, *, assume_spd)` | 从调用方提供的 row-major f32 inverse block 构造 recordable fixed-linear scalar/small-block preconditioner。 | CPU/CUDA/Vulkan；block size 1-4。Forge 不回读、求逆、正则化或推断 SPD；兼容 numeric update 会 rebind 到缓存 Graph submission。 |
...
| `ti.linalg.experimental.BatchedSolvePlan(operator, batch_size, independent_systems=True, ...)` | 在连续扁平分区上构造同构、相互独立的 f32 CG/PCG plan。 | CPU/CUDA/Vulkan；逐系统 tolerance、status 与 iteration count。recordable A/M 可显式选择 `device_convergent`；默认仍为 host-check 执行。 |
...
| `batch_plan.submit(rhs_flat, initial_guess=None, out=None, pacer=None, lane=None, on_saturation='wait')` | 提交一次 solve 并返回 `SolveSubmission`。 | CUDA/Vulkan 的 `fixed_budget_masked` 或满足 recordable A/M 资格的 `device_convergent`；一个 plan-owned slot；可加入共享 `SubmissionPacer`；精确 generation 与 array 保留到 completion。device-convergent 使用一个 Graph ticket，并在 terminal materialization 时公开精确 stop counter。 |
...
| `operator.statistics()` / `plan.statistics()` | 返回 provider/plan execution 与 workspace 诊断。 | 单系统 GPU plan 在可用时报告 A/M、dot product、vector update work、logical/executed/masked iteration、workspace bytes、action counter、preconditioning side 与 chunk activity。recordable Graph Krylov 还报告两级 reduction 策略、block size、每线程 item 数、partial 数量、fixed scratch bytes 与 encoded window size；batch schema v4 区分 host-check recurrence replay 与完整 solve 的 `device_convergent_replay`，并报告精确 logical stop counter；backend 无法公开的 encoded/masked count 不做推测。diagnostic snapshot 不属于数值结果。 |
