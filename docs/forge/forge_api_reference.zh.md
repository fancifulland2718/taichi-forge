...
| `plan.execution_capabilities()` | 返回 backend/provider 执行策略矩阵、选定的默认 policy 与结构化 unsupported reason。 | CUDA stored f32 CSR/BSR CG/PCG 默认使用可自动升级的 `bounded_convergent`；资格满足时执行精确终止的 conditional Graph，直接请求 `device_convergent` 在不可用时会失败且不做 fallback。 |
