...
provider-neutral MINRES、BiCGSTAB 与 restarted GMRES，以及经过验证的 symbolic
factorization 复用合同。
完整用法与后端矩阵见
...
| `ti.linalg.experimental.SolvePlan(operator, method=..., preconditioner=..., execution_policy=..., check_interval=..., restart=...)` | 构造 persistent CG、PCG、MINRES、BiCGSTAB 或 restarted GMRES plan。 | CPU GMRES 支持兼容的 `f32/f64` host action。CUDA/Vulkan `f32` GMRES 支持 fixed stored 或 compiled A，以及 identity 或兼容的 fixed-linear 右 preconditioner；restart 可为 8、16 或 32，fixed-stored identity cycle 可使用 native replay。MINRES/BiCGSTAB/GMRES 的完整 provider 与 policy 矩阵见详细指南。 |
...
| `operator.statistics()` / `plan.statistics()` | 返回 provider/plan execution 与 workspace 诊断。 | 单系统 GPU plan 在可用时精确报告 A/M、dot product、multi-dot、vector update work、logical/executed/wasted iteration、basis/workspace bytes、preconditioning side 与 chunk build/replay/direct/rebind/invalidation；batch plan schema v4 另行报告 plan-owned recurrence Graph 活动，并明确 A/M provider action 不属于该 replay 范围。diagnostic snapshot 不属于数值结果。 |
