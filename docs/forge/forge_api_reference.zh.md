...
| `ti.linalg.experimental.vector_io_capabilities()` / `view.metadata` | 查询版本化 storage、layout、execution mode 与 indexed topology 合同。 | ndarray 为 direct；受支持 field/view 通过 compiled Graph replay 做 device staging；稳定 raw field binding 复用同一 implicit view；不把 device staging 声明为 zero-copy。 |
