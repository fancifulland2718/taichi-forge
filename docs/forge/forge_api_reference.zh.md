...
with ti.hardware.ray.TriangleScene(vertices, indices) as scene:
    ...
    # 只更新 vertex；vertex 数量与 index topology 保持不变。
...
| `operator.prepare_apply(input, out, *, adjoint=False)` | 返回固定 dense 绑定和 recordable generation 的 `PreparedOperatorPlan`（0.6.3）。 | f32 qualified action；准备不运行数学、不做 field staging。新 numeric/coefficient 需重新 prepare，输入/live field 原位更新可见。`record()` 仅 root，独立保留快照。见 [prepared apply](linear_operator.zh.md#prepared-apply063)。 |
