...
| `ti.linalg.SmallBlockInverseBuilder(block_size, block_count, regularization=0, pivot_tolerance=...)` | 通过 `build()` 或单 dispatch `graph_action()` 在 device 上构造 row-major f32 inverse block。 | CPU/CUDA/Vulkan，size 1-4。逐 block device status：0 成功、1 非有限、2 奇异/病态；失败输出为零。不隐式读回 status 或推断 SPD。 |
