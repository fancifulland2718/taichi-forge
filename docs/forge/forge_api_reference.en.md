...
| `preconditioner.pin()` / `.apply(r, out=None)` / `.metadata` / `.statistics()` | Pin an exact target/action generation pair and apply its native action. | No Python hot-path callback; build/accepted stamps and generation publish/retire/release telemetry. |
| `ti.linalg.experimental.SolvePlan(operator, method=..., preconditioner=..., execution_policy=..., check_interval=...)` | Build a persistent CG, PCG, or CPU BiCGSTAB plan. | Built-in Jacobi/block-Jacobi, a trusted fixed-linear operator, or `PreconditionerPlan`; CUDA/Vulkan support `host_check_every_k` with K=4/8 and absolute/relative tolerance; qualified fixed stored CSR/BSR combinations support native chunk replay. |
...
| `operator.statistics()` / `plan.statistics()` | Return provider/plan execution and workspace diagnostics. | Single-system GPU plans report chunk build/replay/direct/rebind/invalidation. Batched-plan schema v4 additionally reports plan-owned recurrence Graph build/replay/rebind/direct counters and explicitly excludes A/M provider actions from that replay scope; it separates host-asynchronous completion from device-parallel guarantees and reports logical workspace payload. A diagnostic snapshot is not part of the numerical result. |
