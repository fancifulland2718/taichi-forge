...
| `ti.linalg.experimental.vector_io_capabilities()` / `view.metadata` | Inspect the versioned storage, layout, execution-mode, and indexed-topology contract. | Ndarrays are direct; supported fields/views use device staging. Canonical contiguous full fields use native bulk copies on CPU/Vulkan; CUDA and indexed/strided views use compiled Graph replay. Stable raw-field bindings reuse one implicit view. Device staging is not reported as zero-copy. |
