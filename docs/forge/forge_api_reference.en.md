...
| `plan.submit(rhs, initial_guess=None, out=None, pacer=None, lane=None, on_saturation="wait", telemetry=False, workspace_lane=None)` | Submit one complete recordable f32 CG/PCG solve and return `SolvePlanSubmission`. | Reuses a cached one-action Graph and one backend ticket. `wait()` does not read terminal state; `result()` materializes it once. Configured workspace lanes own independent Krylov storage. |
