...
| `ti.linalg.experimental.SolvePlan(operator, method=..., preconditioner=..., execution_policy=..., check_interval=...)` | Build a persistent CG, PCG, or CPU BiCGSTAB plan. | Built-in Jacobi/block-Jacobi, a trusted fixed-linear operator, or `PreconditionerPlan`; CUDA/Vulkan support `host_check_every_k` with K=4/8 and absolute/relative tolerance. |
