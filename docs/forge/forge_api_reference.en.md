...
| `operator.statistics()` / `plan.statistics()` | Return provider/plan execution and workspace diagnostics. | Single-system GPU plans report chunk build/replay/direct/rebind/invalidation; batched-plan schema v3 separates host-asynchronous completion from device-parallel guarantees and reports logical workspace payload. A diagnostic snapshot is not part of the numerical result. |
