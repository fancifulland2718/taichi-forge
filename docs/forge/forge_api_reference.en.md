...
| `ti.linalg.experimental.vector_io_capabilities()` / `view.metadata` | Inspect the versioned storage, layout, execution-mode, and indexed-topology contract. | Ndarrays are direct; supported fields/views are device-staged. Device staging is not reported as zero-copy. |
| `operator.apply(x, out=None, *, alpha=1, beta=0, addend=None)` / `operator @ x` | Synchronously compute `out = alpha * A(x) + beta * addend`. | Scalar one-dimensional ndarray or supported field/view; general coefficients on CPU; overwrite on CUDA/Vulkan; `beta=0` does not read addend; no input/output alias. |
