...
| `plan.execution_capabilities()` | Return the backend/provider policy matrix, selected default, and structured unsupported reason. | CUDA stored f32 CSR/BSR CG/PCG defaults to auto-upgrading `bounded_convergent`; qualified runtimes use an exact conditional Graph, while direct `device_convergent` requests fail without fallback when unavailable. |
