...
| CUDA graph-capture stream 隔离 | 将内部 current stream 改为线程局部状态，并且只串行化临时重定向该 stream 的 CUDA graph-capture 事务。 | 一个 Python 线程上的 CUDA graph capture 不会再把另一线程的普通 CUDA 或 CUB 工作错误提交到 capture stream；常规 launch 保持既有异步语义。 |
