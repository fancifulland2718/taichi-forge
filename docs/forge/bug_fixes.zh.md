...
| CPU 并发 job 与 cached graph replay | 将共享 CPU ThreadPool 的一次 job 从设置参数到全部 worker 完成视为完整生命周期；native primitive pool 扩容时保活旧 pool；共享 compiled-graph JIT cache 以完整 replay 事务执行。任意 worker 内嵌套工作会在当前宿主线程以逻辑 worker 0 串行完成，避免自等待和跨 pool 锁环。 | 独立 Python 调用者可以安全共享现有 CPU scheduler 与 graph cache。该项是安全 containment：共享同一 cache/pool 的并发工作可能暂时串行，但不会再覆盖其他调用者的 callback、参数或 cached launch 状态。 |
| CUDA graph capture 与 graph run state | CUDA graph capture 现在将每个 executable 自己的 capture stream 显式传给 CUDA kernel module launch，不再临时重定向 current stream。capture-owned argument buffer 也在同一 stream 上退役。Python graph runtime argument 改为每次 invocation 独立持有；只有 executable 的可变 replay cache 仍被共享并串行化。 | capture、recapture 与普通 CUDA 工作不再依赖临时的 global/TLS stream 路由。复用同一 graph 但传入不同 ndarray 时，不会覆盖另一 invocation 的 flattened argument。现有 graph replay 仍在 default CUDA stream 上保持顺序；普通 CUB/allocator 调用也会在 capture 之外显式选择该 default stream。 |
