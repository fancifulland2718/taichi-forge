...
| `kernel_specialization_limit` | `1024` | Resident `@ti.kernel` specialization/executable-template budget for the current Program generation. Existing specializations keep running at the positive limit, while new cache misses fail clearly. Destroying an SNodeTree returns unpinned generation bindings. Direct root-dense Field templates on CPU/CUDA/Vulkan can reuse an executable when the layout and kernel semantics match, rebuilding only a validated generation binding. Sparse SNodes, hidden field captures, grad/dual, data-oriented templates, and layout mismatches remain generation-compiled. A stale Graph is never retargeted. `ti.reset()` creates a new Program generation. |
