...
| OptiX ABI 93/105 | Registered source-provider ABI | User SDK and source build | `ti.hardware.probe("optix", library_path=...)` | Explicit scene/launch or root Graph; not kernel-callable |
...
Forge does not download the OptiX SDK and does not install the optional
provider. The current source target accepts only OptiX ABI 93 (OptiX 8.1) and
ABI 105 (OptiX 9.0). Do not point it at a newer SDK merely because the driver
supports that SDK; an unsupported header ABI fails configuration by design.

Download a qualified SDK from NVIDIA, install a driver satisfying that SDK's
release notes, and build the provider outside the official wheel:
...
The result is named `taichi_forge_optix_provider_abi1` with the platform shared
library suffix. There is deliberately no install rule. Bind the exact result:

```python
import taichi_forge as ti

# Use the exact .dll on Windows or .so on Linux.
