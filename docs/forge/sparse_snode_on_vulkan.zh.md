...
> **当前 Vulkan 稀疏 SNode 在两条语义合同上偏离 LLVM cpu/cuda 后端：**
> 1. ✅ 已修复（0.3.1）：inactive 稀疏 cell 读返回 dtype 零值。
> 2. ⚠️ 仍存在：3D pointer **全激活**（pointer 容器内每个 cell 都被并发首次写）后接 listgen 依赖 kernel 触发 device-lost；属固有限制（GPU warp lockstep 下单 slot ≥64-way 高并发 CAS spin）。**只在该具体模式下出现**，详情与规避见下方 Bug 2。
...
### ⚠️ 已知限制 Bug 2：3D pointer **全激活**触发 device-lost（**稳定性**）
...
**症状**：写入 kernel 完成（`atomic_add` 计数正确）但下一个 dispatch 触发 `RHI Error: (-4) vkQueueSubmit failed` / Windows 下进程 abort code `0xC0000409`。

**根因（信息性）**：本 fork 的 pointer activate 协议为 *CAS-marker + spin-on-loss*（用于把 race-inflation 下的 cell 计数从 12.5% 提升到 100%）。SPIR-V `OpLoopMerge` + GPU warp lockstep 下，当 ≥64 个 lane 同时争用同一 slot 时，已 win 的 lane 与 spin lane 在同一 warp 被 lockstep 阻塞 → spin 死循环 → driver TDR。根治需要把协议改为 host-enumerate-then-batch，工作量大，**短期不计划修复**。

**已验证安全的模式**（可放心使用）：

- ✅ 1D / 2D pointer 全填（不论尺寸）；
- ✅ 3D pointer 稀疏激活（典型 sparsity ≤ 25%，例如 64³ grid 仅激活 8³ cell）；
- ✅ 3D pointer brick-scatter 模式（外层 ndrange 在 block 坐标上、内层 `ti.static` 序列在 block 内 voxel 上）—— MPM-128、SPH、流体 brick 等地道工作流；
- ✅ **嵌套 pointer**：`pointer.pointer.dense` 二层结构与单层 pointer 行为一致（已三后端字节等价验证）；C-1 hint（见 §3.7）按层独立生效。

**规避方案 1：brick-scatter（推荐）**
...
**规避方案 2：暂时 opt-out 到 vanilla 行为**

```python
ti.init(arch=ti.vulkan, vulkan_sparse_experimental=False)
# 此时 ti.root.pointer 在 Vulkan 上回退到 vanilla 1.7.4 行为（TI_NOT_IMPLEMENTED）；
# 改用 ti.cpu / ti.cuda 后端运行同代码即可。
```

### 当前对 Vulkan 稀疏 SNode 的诚实推荐

- ✅ 适合：MPM / SPH / brick 渲染等 atomic_add scatter（每线程一 cell 的写入分布，很少 ≥64-way 同 slot 争用）；少量 cell 激活后的密集计算；离线 / 带 host 同步的小规模 demo；嵌套 pointer 树（如 OpenVDB-like B+ 树）。
- ⚠️ 暂不适合：3D pointer **全激活**的"先 fill 后 reduce"工作流（受 Bug 2 阻塞）—— 改成 brick-scatter 或切到 cpu / cuda 后端。
...
| `tests/p4/g10p1_user_repro.py` | Bug 2 全激活 device-lost 回归 guard |
