...
> Vulkan sparse SNode 支持在历史 Forge 0.3.0–0.3.2 版本中逐步引入，在 0.3.12 前后完善，并于 0.3.13 加入实验性 `hash`。本文说明当前 0.6.2 源码合同，不表示这些能力都是 0.6.2 新增。受 PyPI 项目容量限制，部分历史 wheel 可能已不再保留。
