...
Install the Taichi Forge Python package from PyPI. For AOT/C-API deployment,
build the C API/TiRT artifacts from the same source release because the public
Forge shim wheel intentionally does not bundle the C API package tree. See
[Building Forge wheels](../../../forge/build_wheels.en.md) for the packaging
boundary.
...
In the C++ wrapper we provide these convenient read/write() methods on NdArray class. See [`c_api/include/taichi/cpp/taichi.hpp`](../../../../c_api/include/taichi/cpp/taichi.hpp).
