...
def _source_values(n, np_dtype):
    if np.issubdtype(np_dtype, np.floating):
        return ((np.arange(n, dtype=np.float32) % 67) - 33) * np.float32(0.25)
...
def test_experimental_gather_scatter_cuda_device_ndarray_i32_f32():
...
def test_experimental_gather_scatter_vulkan_native_ndarray_i32_f32():
    prog = impl.get_runtime().prog
    if not (
...
def test_experimental_gather_scatter_cpu_native_ndarray_i32_f32():
    for dtype, np_dtype in [(ti.i32, np.int32), (ti.f32, np.float32)]:
        gather_ws = _run_ndarray_indexed_copy(dtype, np_dtype, "auto", scatter=False)
