...
import pytest
from taichi_forge.linalg import FieldLinearOperator, MatrixFreeCG
...
@test_utils.test(arch=[ti.cpu, ti.cuda, ti.vulkan], exclude=[vk_on_mac])
def test_matrixfree_cg_uses_absolute_residual_norm_at_zero_iterations(
    rhs_value, expected
):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (-1.0, 0, "tol must be a finite positive number"),
        (float("inf"), 0, "tol must be a finite positive number"),
        (float("nan"), 0, "tol must be a finite positive number"),
        (1e-3, -1, "maxiter must be a non-negative integer"),
        (1e-3, 1.5, "maxiter must be a non-negative integer"),
        (1e-3, True, "maxiter must be a non-negative integer"),
    ],
)
@test_utils.test(arch=[ti.cpu])
def test_matrixfree_cg_rejects_invalid_solver_controls(
    tol, maxiter, message
):
    with pytest.raises(RuntimeError, match=message):
        MatrixFreeCG(None, None, None, tol=tol, maxiter=maxiter)
